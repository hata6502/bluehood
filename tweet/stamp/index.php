<?php
	...
	mysql_start();
	$res = mysql_fetch_assoc(mysql_query("select draw_autosave from user where id=".$_SESSION['twitter']['id']));
	?><script>var is_autosave = <?php echo $res['draw_autosave']; ?>; </script><?php
	mysql_close();
?>
...
var preview_zoom = 2;
	$(function(){
		...
		var edit = function(x, y, begin_x, begin_y){
			x = Math.floor(x/zoom);
			y = Math.floor(y/zoom);
		...
		if (is_autosave){
			...
			alert("スタンプの自動下書き保存はオフになっています。設定でオンにできます。");
		}

		var save_draft = function(){
			$.post(tweet_url+'backupst.php', {stamp: JSON.stringify(edit_data)}, function(res){
			}, 'html').fail(function(jqXHR, textStatus, errorThrown){
				alert("スタンプの自動下書き保存に失敗しました。ページを再読込してください。");
			});
		};
		...
		<div class="main" style="text-align: center; ">
			...
			<div id="phase-1" style="position: relative; ">
				...
				<form method="post" action="upstamp.php">
					<textarea name="text" rows="4" style="width: 80%; " placeholder="#bluehood_stamp " maxlength="100">#bluehood </textarea><br>
