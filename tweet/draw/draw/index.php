...
　
		...
		<div id="stamp-dialog" title="<?php l($s['stamp']); ?>">
			<?php
				try{
					mysql_start();
						...
						?>
							<div style="display: inline-block; vertical-align: top; ">
								...
								<div id="stamp-<?php echo $i; ?>" src="<?php echo $image_url; ?>">
									<img src="<?php echo $image_url; ?>" style="max-width: 240px; ">
								</div>
							</div>
						<?php
						$i++;
...
if (canvas.getContext){
	...
	if (is_autosave){
		...
		alert("お絵かきの自動下書き保存はオフになっています。設定でオンにできます。");
	}
	...
	var save_draft = function(){
		...
		'html').fail(function(){
			alert('お絵かきの自動下書き保存に失敗しました。ページを再読み込みしてください。');
		});
	};
}
});
