<?php
	...
	function detect($file, $selalbum = null, $entity_id = 0){
	...
	if ($filename){
		...
		$image = file_get_contents($status->extended_entities->media[$entity_id]->media_url_https);
		$_SESSION['post_image'] = base64_encode($image);
		
		foreach($status->entities->hashtags as $hashtag){
			$append_text .= '#'.$hashtag->text.' ';
		}
	}else{	// 画像を外す
